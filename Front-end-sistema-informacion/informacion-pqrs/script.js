...
async function cargarTabla() {
  ...
  try {
    const response = await fetch(`${url}/pqrss`);
    ...
    users.forEach(user => {
      ...
      tableRow.innerHTML = `
        <td id="nombre">${user.nombre}</td>
        <td id="empresa">${user.empresa}</td>
        <td id="correo">${user.correo  }</td>
        <td id="telefono">${user.telefono || ''}</td>
        <td id="tipo_peticion">${user.tipo_peticion || ''}</td>
        <td id="mensaje">${user.mensaje || ''}</td>
